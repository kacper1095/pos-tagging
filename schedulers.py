...
def cosine_decay_with_warmup(global_step: int,
    ...
    result = where(global_step > total_steps, minimal_lr, learning_rate)
    return result


class SuperConvergence(optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, total_steps, max_lr,
                 warmup_learning_rate, warmup_steps, minimal_lr=0):
        self._total_steps = total_steps
        self._max_lr = max_lr
        self._warmup_lr = warmup_learning_rate
        self._warmup_steps = warmup_steps
        self._hold_base_rate_steps = minimal_lr
